...
// Updates tda account
func UpsertTDAccount(userId int, portId int, clientId string, refreshToken string) error {
	refreshCipher, err := sundress.BdcEncrypt(refreshToken)
	if err != nil {
		return err
	}
	...
	_, err = db.Exec(`
		INSERT INTO tda_accounts (user_id, port_id, client_id_hash, client_id_cipher, refresh_token_cipher) 
		VALUES ($1,$2,$3,$4,$5)
		ON CONFLICT (user_id, client_id_hash) DO UPDATE
		SET client_id_cipher=$3,refresh_token_cipher=$4`,
		userId, portId, clientIdHash[:], clientIdCipher, refreshCipher)
	return err
}
...
func FetchTDAccount(id int, userId int) (*TDAccount, error) {
	rows, err := db.Query("SELECT id, user_id, port_id, client_id_cipher, refresh_token_cipher FROM tda_accounts WHERE id=$1 and user_id=$2",
		id, userId)
	if err != nil {
		return nil, err
	}
...
func FetchTDAccountsByUserId(userId int) ([]TDAccount, error) {
	rows, err := db.Query("SELECT id, user_id, port_id, client_id_cipher, refresh_token_cipher FROM tda_accounts WHERE user_id=$1", userId)
	if err != nil {
		return nil, err
	}
...
func fetchTDAccountFromRows(rows *sql.Rows) (*TDAccount, error) {
	...
	err := rows.Scan(&td.Id, &td.UserId, &td.PortId, &clientIdCipher, &refreshTokenCipher)
	if err != nil {
		return nil, err
	}
